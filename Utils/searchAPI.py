...
class SpotifyAPI:
    ...
    features2 = [
        "track.id", "track.release_year", "artist.id", "artist.name", "track.name", "track.popularity", "artist.popularity",
        "artist.genres", "track.explicit",

        "trackAttributes.duration_ms", "trackAttributes.tempo", "trackAttributes.mode", "trackAttributes.key",
        "trackAttributes.time_signature", "trackAttributes.loudness",
        "trackAttributes.acousticness", "trackAttributes.danceability", "trackAttributes.energy",
        "trackAttributes.instrumentalness", "trackAttributes.liveness",
        "trackAttributes.speechiness","trackAttributes.valence"
    ]
    ...
    def getTrackData(self, trackID, megaDict, getAddedAt=True):
        arr = []
        artistID = megaDict["track"][trackID]
        f = SpotifyAPI.features
        if not getAddedAt: f = SpotifyAPI.features2
        for f in f:
            if f == "addedAt":
                arr.append(megaDict["addedAt"][trackID])
            if f == "track.release_year":
                arr.append(megaDict["track"][trackID]["album"]["release_date"][:4])
            else:
                obj, attr = f.split(".")
                arr.append(megaDict[obj][trackID][attr])
        return np.array(arr)

    def __formatResults__(self, tracksDict, addedAt, featuresDict, artistDict, getAddedAt=True):
        megaDict = {}
        megaDict["track"] = tracksDict
        if getAddedAt: megaDict["addedAt"] = addedAt
        megaDict["trackAttributes"] = featuresDict
        megaDict["artist"] = artistDict
        tracks = np.array([self.getTrackData(t, megaDict, getAddedAt) for t in tracksDict.keys()])
        if getAddedAt: df = pd.DataFrame(tracks, columns=SpotifyAPI.features)
        else: df = pd.DataFrame(tracks, columns=SpotifyAPI.features2)
        return df
    ...
    def search(self, query, type="track", **kwargs):
        q = self.api.search(query=query, type=type, **kwargs)
        tracks = [t for t in q.tracks.items]
        ids = [t.id for t in tracks]
        tracksDict = self.__addTracksToDict__(q.tracks.items, {}, lambda t: t.id, lambda t: t)
        featsDict = self.__loadTrackFeatures__([t.id for t in tracks])
        artistDict = self.__getArtists__(tracksDict)

        df = self.__formatResults__(tracksDict, None, featsDict, artistDict, False)

        c = [
       'trackAttributes.mode',
       'trackAttributes.acousticness', 'trackAttributes.danceability',
       'trackAttributes.energy', 'trackAttributes.instrumentalness',
       'trackAttributes.liveness', 'trackAttributes.speechiness', "trackAttributes.valence"]
        df = df.set_index("track.id")
        df["track.release_year"] = df["track.release_year"].astype(np.float64)
        df = df.loc[ids, :]
        df = df.reset_index(0)
        df[c] = df[c].astype(np.float64)
        df[c] *= 1000
        newColNames = ['id', 'year', 'artist.id', 'artist.name',
       'track.name', 'track.popularity', 'artist.popularity', 'artist.genres',
       'track.explicit', 'trackAttributes.duration_ms',
       'tempo', 'mode', 'key',
